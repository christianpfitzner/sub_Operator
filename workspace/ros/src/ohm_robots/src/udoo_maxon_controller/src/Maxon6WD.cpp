...
Maxon6WD::Maxon6WD()
{
  ...
  _velSub = _nh.subscribe("vel/teleop", 1, &Maxon6WD::velocityCallback, this);
}
...
void Maxon6WD::velocityCallback(const geometry_msgs::Twist& cmd)
{
  double vl, vr;
...
void Maxon6WD::twistToTrackspeed(double *vl, double *vr, double v, double omega) const
{
  *vr =  -1 * (v - omega * _diagonal / (2.0 * _cosa));
  *vl =       (v + omega * _diagonal / (2.0 * _cosa));
}

void Maxon6WD::trackspeedToTwist(double vl, double vr, double *v, double *omega) const
{
  *v     = (vl - vr) / 2.0;
  *omega = (vr + vl) * _cosa / (2.0 * _diagonal);
}

double Maxon6WD::trackspeedToTicksPerTurn(double v) const
{

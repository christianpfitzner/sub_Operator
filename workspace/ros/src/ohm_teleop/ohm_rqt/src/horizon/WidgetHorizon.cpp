...
WidgetHorizon::WidgetHorizon(void)
{
  pitch = 0;
  roll = 0;
}
...
void WidgetHorizon::paintEvent(QPaintEvent* event)
{
  this->setFixedHeight(this->width());      // adapt window height to width, so ellipse appears as circle
  WidgetHorizon::paintHorizon(pitch,roll); // paint the virtual horizon (blue-green background)
  WidgetHorizon::paintMask();               // paint mask (circle and lines)
  WidgetHorizon::paintScale();              // paint scale for the mask
  WidgetHorizon::paintCross(pitch, roll);  // paint floating red cross
}


void WidgetHorizon::paintHorizon(float pitch, float roll)
{
  QPainter painter(this);
  QPen pen(Qt::SolidLine);
  QPolygon poly1, poly2;
  QColor blue(150, 150, 255);
...
void WidgetHorizon::paintMask(void)
{
  QPainter painter(this);
  QPen pen(Qt::SolidLine);
...
void WidgetHorizon::paintCross(float pitch, float roll)
{
...
void WidgetHorizon::paintScale(void)
{
  float width = this->width();
  QPainter painter(this);
...
void WidgetHorizon::setPitch(float p)
{
...
void WidgetHorizon::setRoll(float r)
{

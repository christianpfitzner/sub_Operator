...
#define TRYSLEEP  (unsigned int)5500 //3200 //1e5 //best: 5500
...
public:
   ...
   endless loop. Returns after target position has been reached */
    ...
    int getActualPosition(long *apos, BYTE nodeAddress);
    
	long getActualTravelledDistance(long offset, BYTE nodeAddress);

	/*! \brief Reads the absolute encoder ticks for given motor id */
	long getAbsoluteEncoderTicks(BYTE nodeAddress);

	/*! \brief Reads the relative encoder ticks for given motor id */
	long getRelativeEncoderTicks(BYTE nodeAddress);

    /*! \brief read actual position; 14.1.67 */
    int getDemandVelocity(long *dvel);

    /*! \brief read actual position; 14.1.68 */
    int getActualVelocity(long *avel, const BYTE NodeId = 0x01);
	...
	bool InitDevice(char *device_adresse);

/*
 * File:   CEposVMC.cpp
 * Author: viatcheslav tretyakov, marcel dombrowski, 
 *			thomas gisder, stefan wilkes
 *
 * Created on November 10, 2009, 3:44 PM
 * Last modification on November 6, 2010, 12:49 PM
 */
#include "CEposVMC.h"
#include <string.h>
#include <ctype.h>
...
CEposVMC::CEposVMC()
{
	...
	/* Speicherbereich fuer Encoder Referenzadressen vorbereiten */
	this->encoder_reference_values = 
		(long *)malloc(sizeof(long) * nodesQuantity);
}
...
bool CEposVMC::InitDevice(char *device_adress)
{
	// check wether connection is made via TCP/IP or ser. device

	// is argument starting with a '/'?
	if (!strncmp(device_adress, "/", 1))
	{
		printf("trying to open '%s'...", device_adress);
		if (openEPOS(device_adress) < 0)
		{
	...
	else if (isdigit(device_adress[0]))
	{
		//    printf("%s\n", arg[1]);
		string_pos = strspn(device_adress, "1234567890.");
		if (string_pos == strlen(device_adress))
		{
			fprintf(stderr, "TCP/IP PORT NOT GIVEN!\n");
			return false;
		}
		//    fprintf(stderr,">>>%d<<<\n", string_pos);
		strncpy(ip, device_adress, string_pos);
		ip[string_pos] = '\0';
		//printf("%s; %d  -- >%s<\n", ip, string_pos, strpbrk(arg[1],":"));

		if (sscanf(strpbrk(device_adress, ":"), ":%hu", &port) == EOF)
		{
			fprintf(stderr, "TCP/IP PORT NOT GIVEN!\n");
			return false;
		}
	...
	do
	{
		...
		if (n == 11)
		{
			...
			changeEPOSstate(6, ID);

			// 1b. check status
			if (checkEPOSstate(ID) == 11)
			{
		...
		else
		{
			...
			//cout << "change state of node: " << (int) ID << endl;
			changeEPOSstate(2, ID); // now in state 'switch on disabled'
		}
		...
		if (n != 2)
		{
			...
			printf("EPOS is still NOT in 'switch on disabled' state, but in %d -> quit!\n", n);
		...
		// issue a 'shutdown'
		//cout << "change state of node: " << (int) ID << endl;
		changeEPOSstate(0, ID); // should now be in state 'ready to switch on (3)'
		...
		printf("\nswitching on... ");
		//cout << "change state of node: " << (int) ID << endl;
		changeEPOSstate(1, ID); // should now be in state 'switched on (4)'
		...
		/* Start Referenz-Wert laden */
		this->encoder_reference_values[(int)ID] = 
				this->getAbsoluteEncoderTicks(ID);
 ...
 */
int CEposVMC::openEPOS(char* device)
{ //pure rs232
	...
	if (fcntl(ep, F_SETFL, FNDELAY) < 0)
	{ //FNDELAY enyspricht grob O_NONBLOCK
		perror("fcntl");
		return (-1);
	}
...
int CEposVMC::openTCPEPOS(char* ip, short unsigned port)
{
 ...
 (file %s, line %d)\n\n", inet_ntoa(
				address.sin_addr), port, __func__, __FILE__, __LINE__);
		return (-1);
	}
...
int CEposVMC::getActualPosition(long* apos, BYTE nodeAddress)
{
	WORD *answer = NULL;
	int n = 0;
	...
	if ((n = ReadObject(0x6064, nodeAddress, 0x00, &answer)) < 0)
	{
...
//long ticsRight = 0, ticsLeft = 0;

long CEposVMC::getActualTravelledDistance(long offset, BYTE nodeAddress){
    long current_position, tics;
    getActualPosition(&current_position, nodeAddress);
    tics = current_position - offset;
/*
    if (nodeAddress == 0x01) {
     ticsRight += tics;
     tics       = ticsRight;
...
long CEposVMC::getAbsoluteEncoderTicks(BYTE nodeAddress)
{
	long encoder_ticks;

	/* Konnten die Steps gelesen werden, so sind die Encoder Ticks ein viertel 
	 * dieser (QuadCount), sonst 0 */
	encoder_ticks = (this->getActualPosition(&encoder_ticks, nodeAddress) != -1)
						? encoder_ticks / 4
						: 0;
...
long CEposVMC::getRelativeEncoderTicks(BYTE nodeAddress)
{
	long abs_encoder_ticks = this->getAbsoluteEncoderTicks(nodeAddress);
	long rel_encoder_ticks = abs_encoder_ticks - 
			this->encoder_reference_values[(int)nodeAddress];

	this->encoder_reference_values[(int)nodeAddress] = abs_encoder_ticks;

	return rel_encoder_ticks;
}
...
int CEposVMC::getActualVelocity(long* avel, const BYTE NodeId)
{
	WORD *answer = NULL;
	int n = 0;
...
int CEposVMC::moveAbsolute(long int steps, long int speed, int acceleration, int deceleration, int maxVelocity)
{
...
int CEposVMC::profileVelocityMode(long int targetVelocity, int maxVelocity, int acceleration, int deceleration)
{
...
int CEposVMC::setRPM(long int targetVelocity1, long int targetVelocity2, BYTE nodeAddress1, BYTE nodeAddress2)
{
	// check, if we are in Profile Velocity Mode
...
int CEposVMC::monitorStatus()
{
	...
	do
	{
		...
		printf("\rEPOS: pos=%+10ld |%+10ld (%ld to go); v= %+4ld | %+4ld[rpm]; I=%+4dmA", postarget, posactual,
				postarget - posactual, veldemand, velactual, curactual);
		fflush(stdout);
...
int CEposVMC::monitorHomingStatus()
{
	...
	do
	{
		...
		printf("\r%d EPOS: pos=%+10ld; v =  %+4ldrpm I=%+3dmA status = %#06x ", i, posactual, velactual, curactual,
				status);

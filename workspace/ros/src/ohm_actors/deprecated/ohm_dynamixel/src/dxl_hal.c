...
int dxl_hal_open(const char* device, const float baudrate)
{
    struct termios newtio;

    gfByteTransTime = (float)((1000.0f / baudrate) * 12.0f);

    strcpy(gDeviceName, device);
...
int dxl_hal_tx(const unsigned char *pPacket, int numPacket)
{
    return write(gSocket_fd, pPacket, numPacket);
}

int dxl_hal_rx(unsigned char *pPacket, const int numPacket)
{
...
void dxl_hal_set_timeout(const int NumRcvByte)
{
    glStartTime = myclock();
